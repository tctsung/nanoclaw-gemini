...
function buildOpenCodeConfig(options: ProviderOptions): Record<string, unknown> {
  ...
  const providerFallbackModelId = fallbackModel ? fallbackModel.replace(new RegExp(`^${provider}/`), '') : undefined;
  const modelsToRegister = [providerModelId, providerSmallModelId, providerFallbackModelId]
    .filter(Boolean)
    .filter((mid, i, a) => a.indexOf(mid as string) === i);

  const optionsForProvider: Record<string, string> = {};
  if (provider === 'google' && googleApiKey) {
    optionsForProvider.apiKey = googleApiKey;
  } else {
    optionsForProvider.apiKey = 'placeholder';
  }
  if (proxyUrl) optionsForProvider.baseURL = proxyUrl;
...
function runtimeConfigKey(options: ProviderOptions): string {
  return JSON.stringify({
    ...
    fallback: process.env.OPENCODE_FALLBACK_MODEL,
