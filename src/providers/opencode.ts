...
import { readEnvFile, readUserEnvFile } from '../env.js';
...
registerProviderContainerConfig('opencode', (ctx) => {
  ...
  const userDotenv = readUserEnvFile(keys);
  const projectDotenv = readEnvFile(keys);
  ...
  for (const key of [
    ...
    const value = ctx.hostEnv[key] || projectDotenv[key] || userDotenv[key];
